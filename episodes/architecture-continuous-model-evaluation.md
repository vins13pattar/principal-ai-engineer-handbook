# Continuous Model Evaluation: Why Most Eval Platforms Produce Numbers, Not Decisions

_An evaluation platform's only job is to support a comparison, and every design choice in this architecture exists to keep a number from being mistaken for a decision — from the statistics that make small deltas invisible, to the graders that can lie, to the cache sitting silently underneath the whole pipeline._

- **Source:** [architecture:continuous-model-evaluation](/architecture/systems/continuous-model-evaluation/)
- **Runtime:** 15:30 · 43 turns · 12 beats
