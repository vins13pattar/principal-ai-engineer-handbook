...
**Wall-clock here is noisy across runs in a way the byte columns are not.** The byte tables above
are bit-for-bit identical between repeat runs on the same machine. This one is not: a later run of
the same command on the same machine put the `snapshot_frequency=10000` row at 3.69 ms rather than
0.20 ms. What reproduces is the *ordering* — never snapshotting is always the slowest row, because
it is the deepest replay. Do not read a magnitude off this table.
...
- The channel's own docstring states that its API and its **on-disk representation may change**.
  Checkpoints it writes are not guaranteed to stay readable across LangGraph versions the way
  checkpoints from stable channels are.
- The contract this lab had to reach for to measure it — `get_delta_channel_history`, the
  `_DeltaSnapshot` blob shape, `counters_since_delta_snapshot` — is not a stable public surface
  either. Anything reading delta state back out is coupled to internals.
...
None of that makes it the wrong choice. It makes it a choice that needs a migration plan for the
checkpoint store, which "the fix" does not communicate.
...
- **"The fix" is a migration.** Adopting `DeltaChannel` late means rewriting the state model *and*
  owning a checkpoint format that is documented as unstable. Reaching for it early costs almost
  nothing; reaching for it under load costs a migration you did not schedule.
