# ADR-0008: Choosing TypeScript and a Plain Pipeline Over Python and LangGraph

_A podcast-generation pipeline inherited two design assumptions from outside the repo — Python and LangGraph — and this ADR tests both against what the repository actually is: eleven isolated Python labs, a TypeScript content stack that already owns the schemas, and a workflow that turns out to be a single linear loop rather than a graph._

- **Source:** [adr:0008-typescript-podcast-pipeline](/adr/decisions/0008-typescript-podcast-pipeline/)
- **Runtime:** 12:07 · 30 turns · 8 beats
