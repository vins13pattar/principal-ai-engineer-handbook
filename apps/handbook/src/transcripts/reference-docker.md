...
**Guest:** Right — the cache rule is brutal and simple: the first changed layer invalidates itself and every single layer after it, no matter how trivial the change was. So the classic mistake is copying your entire source tree first, then installing dependencies. Now every source edit, even a comment change, reinvalidates the dependency install, and you're reinstalling packages that never changed. Flip it — copy just the lockfile, install dependencies, then copy source — and that expensive layer stays cached, since layers are reused whenever the instruction and its inputs are unchanged. And none of that helps if your build context itself is huge, which is why a missing .dockerignore is usually the actual reason someone's build is slow: the whole directory, node_modules and all, gets shipped to the daemon before the build even starts.
