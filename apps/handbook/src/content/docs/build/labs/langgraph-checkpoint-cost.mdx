...
100 steps across 5, 25, 100, and 10000 — the last being large enough that no snapshot is ever
written after the first, so every resume replays the full run.

**This page deliberately publishes no table of those timings, and that absence is the result.**
The three byte tables above are bit-for-bit identical across all **eleven** runs on record, produced
by three independent agents. The resume timings are not, and two earlier drafts of this page were
...
time. What eleven runs do support is two claims, and the section rests on them:

- **Scale.** Every configuration resumes in **tens to low hundreds of microseconds** at 100 steps —
  well under a millisecond, in every run. That is the honest size of `DeltaChannel`'s deferred cost:
  real, measurable, and small at this scale.
...
- **"The fix" is a state-model rewrite, and the beta risk is narrower than it sounds.** Adopting
  ...
  The storage risk is the smaller half: LangGraph says threads already written are *expected* to
  remain readable — an expectation, not a guarantee — and what it names as unstable is the
  surrounding contract, which only bites you if you read delta state yourself instead of through
  `get_state`. Know which of the two risks you are actually taking, and how firm the assurance under
  it is, before you quote either to a review.
